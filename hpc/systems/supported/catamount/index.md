...
CATAMOUNT cluster uses [SLURM](https://scienceit-docs.lbl.gov/hpc/running/slurm-overview/index.md) as the scheduler to manage jobs on the cluster. To use the CATAMOUNT resource, the partition `catamount` must be used (`--partition=catamount`) along with account `catamount` (`--account=catamount`). One of the QoSs from the following table should be used as well (e.g., `--qos=cm_short`). A standard fair-share policy with a decay half life value of 14 days (2 weeks) is enforced.
...
CATAMOUNT uses [Software Module Farm](https://scienceit-docs.lbl.gov/hpc/software/software-module-farm/index.md) to [manage](https://scienceit-docs.lbl.gov/hpc/software/module-management/index.md) the cluster-wide software installation.
